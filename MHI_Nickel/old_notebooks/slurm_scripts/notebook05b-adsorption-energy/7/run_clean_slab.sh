#!/bin/bash
#SBATCH --job-name=nb05b_clean_slab
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --gres=gpu:a100:1
#SBATCH --partition=multigpu
#SBATCH --time=02:00:00
#SBATCH --output=results/notebook05b-adsorption-energy/7/slurm_clean_slab_%j.out
...
/projects/westgroup/akinyemi.az/mace_lammps/lammps/build-mliap/lmp -k on g 1 -sf kk -pk kokkos newton on neigh half \
    -in lammps_scripts/notebook05b-adsorption-energy/7/min_clean_slab.lammps \
    -log results/notebook05b-adsorption-energy/7/ads_clean_slab.log

echo "clean_slab done: $(date)"
