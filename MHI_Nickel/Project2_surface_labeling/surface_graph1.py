...
def build_surface_graph(slab_path, seed=7, bond_cutoff=3.2,
    ...
    # Find the correct n_layers for CustomSurface by trying values 1..10
    # CustomSurface requires that its internal plane count is exactly
    # divisible by n_layers — we find the right value by trial
    cs, sas = None, None
    n_layers_acat = None
    for nl in range(1, 11):
        try:
            cs_try = CustomSurface(top3_slab, n_layers=nl)
            cs, n_layers_acat = cs_try, nl
            break
    ...
    if cs is None:
        raise RuntimeError(
            'Could not find a valid n_layers for CustomSurface (tried 1-10). '
            'Check that the slab has at least 3 layers and is periodic in xy.')

    print(f'  n_layers used for ACAT  : {n_layers_acat}')

    sas = SlabAdsorptionSites(top3_slab, surface=cs,
                              composition_effect=True,
                              label_sites=False, tol=acat_tol)
    raw_sites = sas.get_sites()
