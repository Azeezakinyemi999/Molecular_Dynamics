#!/bin/bash
#SBATCH --job-name=Cr_oxide_min
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --gres=gpu:a100:1
#SBATCH --partition=''gpu
#SBATCH --time=08:00:00
#SBATCH --output=results/02_bulk_energy_minimization/Cr_oxide_minimize_%j.log
...
/projects/westgroup/akinyemi.az/mace_lammps/lammps/build-mliap/lmp -k on g 1 -sf kk -pk kokkos newton on neigh half \
    -in lammps_scripts/02_bulk_energy_minimization/Cr_oxide__minimize.lammps \
    -log results/02_bulk_energy_minimization/Cr_oxide_minimization.log
