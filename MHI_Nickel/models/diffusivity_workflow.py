...
# ─────────────────────────────────────────────────────────────────────────────
# Per-metal counterpart -- write_diffusivity_run_script
# ─────────────────────────────────────────────────────────────────────────────

def write_diffusivity_run_script(
    struct_path,
    stem,
    n_h_values,
    temperatures,
    ...
    """Write diffusivity_run_{stem}.py for one metal. Returns the output path.

    Per-metal counterpart to ``generate_diffusivity_scripts`` (which handles
    every metal in a single combined script, one process working through
    them serially). Mirrors ``write_neb_run_script``'s per-metal pattern so
    a metal's diffusivity can run as its own independent, concurrent
    orchestrator process -- same as ``neb_run_{stem}.py`` /
    ...
    _header = f'''#!/usr/bin/env python3
"""
diffusivity_run_{stem}.py
==================
Standalone H-diffusivity workflow orchestrator for {stem}.
Per-metal counterpart to diffusivity_run.py (same phases, one structure --
see write_diffusivity_run_script() in models/diffusivity_workflow.py).

Loop: n_h_values x temperatures (single structure)
  Phase 1a: Minimise input bulk structure
  Phase 1b: NPT MD at each T -> a0(T) -> insert H -> minimise bulk+H
  Phase 2 : NVT MD (chained SLURM GPU jobs, one per temperature)
  Phase 3 : MSD  ->  D(T)  ->  Arrhenius fit

Generated by write_diffusivity_run_script() -- do not edit by hand.
"""

# -- config (injected) ----------------------------------------------------------
STRUCT_PATH      = {struct_path!r}
STRUCT_STEM      = {stem!r}
N_H_VALUES       = {n_h_values!r}
TEMPERATURES     = {temperatures!r}
WORK_DIR         = {work_dir!r}
...
MIN_MAXEVAL      = {min_maxeval!r}
MIN_RESTART_EVERY = {min_restart_every!r}
# Element / type tables for this metal
ELEM_STR         = {elem_str!r}
E2T              = {e2t!r}
MASSES           = {masses!r}
...
try:
    ...
    print(f'\n  *** FAILED (shared bare-bulk/NPT for {struct_stem}): {_exc} ***')
    print(f'  All n_H values for {struct_stem} cannot proceed -- aborting.')
    for _n_h in N_H_VALUES:
        _FAILURES.append({'run_name': f'{struct_stem}_{_n_h}H', 'struct': struct_stem,
                           'n_h': _n_h, 'error': str(_exc)})
    _failures_path = os.path.join(WORK_DIR, f'diffusivity_failures_{struct_stem}.json')
    with open(_failures_path, 'w') as _f:
        _json_lat.dump(_FAILURES, _f, indent=2)
    print(f'  See {_failures_path}')
    sys.exit(1)
...
if _FAILURES:
    ...
    print(f'\n=== All H concentrations complete for {struct_stem} ===')
"""
